...
export default function HelpPanel({ guia, onClose, onStart }) {
  ...
  useDialogoModal(painelRef, onClose)
  ...
  return <div className="fixed inset-0 z-[80] flex items-end justify-center bg-black/65 sm:items-stretch sm:justify-end" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <section ref={painelRef} tabIndex="-1" role="dialog" aria-modal="true" aria-labelledby="help-title" className="max-h-[88vh] w-full overflow-y-auto rounded-t-3xl border border-marinho-borda bg-marinho-claro p-5 shadow-flutuante sm:h-full sm:max-h-none sm:max-w-md sm:rounded-none sm:border-y-0 sm:border-r-0 sm:p-7">
      <div className="flex items-start gap-3"><span className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl bg-ouro-fosco text-ouro"><CircleHelp size={20} /></span><div className="min-w-0 flex-1"><p className="section-label">Ajuda nesta tela</p><h2 id="help-title" className="mt-1 font-display text-2xl font-bold text-texto">{guia.titulo}</h2></div><button type="button" onClick={onClose} aria-label="Fechar ajuda" className="flex h-10 w-10 items-center justify-center rounded-xl text-texto-secundario hover:bg-marinho hover:text-texto"><X size={20} /></button></div>

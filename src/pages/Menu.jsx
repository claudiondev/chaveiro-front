...
export default function Menu() {
  ...
  return <div className="page-shell"><div className="page-content max-w-4xl"><PageHeader titulo="Equipe e administração" subtitulo="Acessos, relatórios e rotinas do Chaveiro Abençoado." />
    <section data-tour="menu-perfil" className="mt-8 flex items-center gap-4 border-y border-marinho-borda py-5"><div className="flex h-12 w-12 items-center justify-center rounded-full bg-ouro font-display text-xl font-bold text-marinho">{inicial}</div><div className="min-w-0 flex-1"><p className="truncate font-medium text-texto">{usuario?.nome}</p><p className="truncate text-xs text-texto-secundario">{usuario?.email}</p></div><span className="text-[10px] font-bold uppercase tracking-wider text-ouro">{usuario?.role === 'DONO' ? 'Dono' : 'Funcionário'}</span></section>
    <div className="mt-8 grid gap-8 md:grid-cols-2"><section data-tour="menu-operacao"><h2 className="section-label">Operação</h2><div className="mt-3 divide-y divide-white/5 border-y border-marinho-borda"><Item icon={ListChecks} label="Tabela de serviços" descricao="Preços do balcão e externos" onClick={() => navigate('/servicos')} />{isDono() && <Item icon={ClipboardList} label="Fechamento do dia" descricao="Conferir o caixa" onClick={() => navigate('/fechamento')} />}</div></section>{isDono() && <section data-tour="menu-administracao"><h2 className="section-label">Administração</h2><div className="mt-3 divide-y divide-white/5 border-y border-marinho-borda"><Item icon={BarChart3} label="Relatórios" descricao="Movimento por período" onClick={() => navigate('/relatorios')} /><Item icon={UserPlus} label="Cadastrar funcionário" descricao="Criar acesso para a equipe" onClick={() => setCadastro(true)} /></div></section>}</div>
    ...
    <button data-tour="menu-sair" onClick={sair} className="mt-10 flex items-center gap-2 text-sm font-semibold text-erro"><LogOut size={16} /> Sair da conta</button>
  </div></div>
}
